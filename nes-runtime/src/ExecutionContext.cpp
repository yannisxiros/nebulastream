...
nautilus::val<TupleBuffer*> ExecutionContext::allocateBuffer() const
{
    auto bufferPtr = nautilus::invoke(
        +[](PipelineExecutionContext* pec)
        {
            ...
            auto* tb = new TupleBuffer(buffer); /// NOLINT(cppcoreguidelines-owning-memory) - ownership managed by operator code
            return tb;
        },
        pipelineContext);
    return bufferPtr;
...
void emitBufferProxy(PipelineExecutionContext* pipelineCtx, TupleBuffer* tb)
{
    ...
    delete tb; /// NOLINT(cppcoreguidelines-owning-memory) - matching new in allocateTupleBuffer
}
