    ...
    const VariableSizedData& variableSizedData)
{
    ...
    *static_cast<nautilus::val<uint32_t*>>(refToIndex) = variableSizedData.getSize();
    if (variableSizedData.getSize() <= VariableSizedAccess::inlineBufSize)
    {
        nautilus::memcpy(getMemberWithOffset<int8_t*>(memoryAddress, offsetof(VariableSizedAccess::StringEntry, prefix)),
                variableSizedData.getContent(), variableSizedData.getSize());
    }
    else
    {
        nautilus::invoke(
            ...
            const uint64_t varSizedDataSize)
        {
            ...
            memcpy(&refToIndex->prefix, varSizedPtr, VariableSizedAccess::prefixSize);
...
void storeVarSized(
    ...
    const nautilus::val<AbstractBufferProvider*>& bufferProviderRef,
    const nautilus::val<int8_t*>& memoryAddress,
    const GermanVarsized& variableSizedData)
{

    auto refToIndex = static_cast<nautilus::val<VariableSizedAccess::StringEntry*>>(memoryAddress);
    nautilus::memcpy(refToIndex, variableSizedData.getReference(), sizeof(VariableSizedAccess::StringEntry));
    ...
    const Record& record) const
{
    for (const auto& [fieldIdentifier, type, fieldOffset] : nautilus::static_iterable(fields))
    {
        ...
        if (type.isType(DataType::Type::VARSIZED))
        {
            ...
            {
                auto varSizedValue = value.cast<GermanVarsized>();
                storeVarSized(hashMapRef, bufferProvider, memoryAddress, varSizedValue);
            }
        }
        else
